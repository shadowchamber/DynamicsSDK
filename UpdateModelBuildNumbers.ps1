<#
.SYNOPSIS
    Update version numbers in model descriptor files.

.DESCRIPTION
    This script is intended for use in the Dynamics AX Development environment
    to automate updating model versions with the build number in the build process.
...
.NOTES
	...
	The build number supplied in -BuildNumber must be of format #.#.#.#
	
	Any models in layers above layer -UpdateLayersAbove will have their descriptors updated.
	Default is all models above ISP (9).

	Optionally specific models can be excluded by providing a comma separated list in -ModelExclusions
...
Param(
    ...
    [Parameter(Mandatory=$true, HelpMessage="The build number in format a.b.c.d")]
    [string]$BuildNumber,
    ...
    [Parameter(HelpMessage="ID of layer of which all layers above it will have their models' version set in the output")]
    [int]$UpdateLayersAbove = 9,

    [Parameter(HelpMessage="Comma separated list of model names that should not have their version updated")]
    [string]$ModelExclusions = ""
)

# Signal model version update start
$InstrumentationScript = Join-Path -Path $PSScriptRoot -ChildPath "DevALMInstrumentor.ps1"
& $InstrumentationScript -TaskSetModelVersionsStart -DataArgument "{ `"BuildNumber`": `"$BuildNumber`", `"LayersAbove`": `"$UpdateLayersAbove`", `"ModelExclusions`": `"$($ModelExclusions.Length)`" }"

# Import module for Write-Message and other common functions (Picks up $LogPath variable).
Import-Module $(Join-Path -Path $PSScriptRoot -ChildPath "DynamicsSDKCommon.psm1") -Function "Write-Message"

[int]$ExitCode = 0
try
{
	Write-Message ("Updating model build numbers...")

	$Exclusions = $ModelExclusions.Split(",")
	...
	if ($BuildNumber -match "^\d+\.\d+\.\d+\.\d+$")
	{
		...
		{
			# Discover the model descriptors
			...
			if (Test-Path -Path $BuildDescriptorPath -PathType Container)
			{
				...
				if ($DescriptorFiles.Count -gt 0)
				{
					...
					{
						[xml]$xml = Get-Content $DescriptorFile.FullName -Encoding UTF8
		
						# Model Names are unique, regardless of Publisher
						$modelName = ($xml.SelectNodes("/AxModelInfo/Name")).InnerText

						if ($Exclusions -icontains $modelName)
						{
							Write-Message "- Skipped updating version of model: $modelName"
						}
						else
						{
							...
							# If layer is above limit (default = 9 = ISP)
							if ($layerid -gt $UpdateLayersAbove)
							{
								...
								$xml.Save($DescriptorFile.FullName)

								Write-Message "- Updated model $modelName version to $BuildNumber"
							}
						}
					}
				}
	...
	{
		throw "Build Number $BuildNumber has wrong format and should be #.#.#.#"
	}

	Write-Message "Updating model build numbers completed."

	# Signal model version update end
	& $InstrumentationScript -TaskSetModelVersionsStop
}
catch [System.Exception]
{
    ...
    Write-Message "Error updating model build numbers: $($_)" -Error
	$ExitCode = -1
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDVH1kQEm2w7GIQ
# jJhKtnFh5LC+nGRL+uLqw0mc+NaTgqCCDYEwggX/MIID56ADAgECAhMzAAAB32vw
...
# BgEEAYI3AgELMQ4wDAYKKwYBBAGCNwIBFTAvBgkqhkiG9w0BCQQxIgQ